...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score <= 5) return 'wdnmd';
            if (score <= 8) return 'SB';
            if (score <= 10)  return 'SB';
            return 'SB';
        } else {
            if (score <= 49) return 'SB';
            if (score <= 99) return 'SB';
            if (score <= 149) return 'SB';
            return 'SB';
        }
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "刺殺彭仲胤";
        if (cookie("keyboard")) {
